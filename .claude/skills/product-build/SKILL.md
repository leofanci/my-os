...
**Left panel tabs ≠ product features.** Each tab holds one artifact type — see `dashboard/ai_rules.py` TAB_ROUTING.

**Write gate:** propose feature list + target Product tab (`sec05`) in chat. Commit via `add-feature --section Next` only after user approves. Never put features in intake or technical. Roadmap features under **Next**, not **Shipped**.
...
**Shipped = checked `- [x]` only.** Unchecked lines under `## Shipped` still show as planned in the UI. New spec work always goes under `## Next` as `- [ ]`.

**Micro-features = one `- [ ]` line** under any section, added by hand. No
ceremony. When done: move to `## Shipped` and flip to `- [x]` with a `shipped:` date.
That's the whole manual loop.
