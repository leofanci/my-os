...
class NumberedArtifactStore:
    ...
    def __init__(self, dir_name: str, id_prefix: str):
        ...
        self._bare_id_re = re.compile(rf"^{re.escape(id_prefix)}\d+$")

    def _safe_id(self, artifact_id: str | None) -> str:
        value = artifact_id or self.default_id
        if not self._bare_id_re.fullmatch(value):
    ...
    def file(self, profile_dir: Path, artifact_id: str | None = None) -> Path:
        return profile_dir / self.dir_name / f"{self._safe_id(artifact_id)}.md"
