...
## Write gate
Propose 2-3 options in chat (target `sec03`). Run `create-experiment` only after user picks one or says save. Never `create-activity` for a proposed test.
...
- Design must fit intake resources (hrs/week, budget). A 40h test for a 5h/week
  project is invalid.
- After save: tell the user to come back with results → experiment-review.
