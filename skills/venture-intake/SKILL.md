...
5. **Write gate:** propose intake sections + which tab each maps to (`sec01` What it is, `sec02` validation sections). Commit via `update-intake` only after user approves. One section group per approval when filling from a large paste.
6. Evidence log at bottom — experiment-review appends dated entries.
7. If material facts changed, recommend rerunning gtm-assessment.
