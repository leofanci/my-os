...
The 23 skills already run **file-based** today. This document is the order to
build the automation around them: the `claude -p` generation pipeline, the
SQLite index, the rebuild hook, and the dashboard. Build against representative
local fixtures, never tracked private data and never ahead of the file model.
...
## Phase 0 — Seed local fixture data (untracked)

Use the file-based workflow to create representative local inputs under the
gitignored `projects/` directory:
